...
public class InquiryApprovalFlow implements AutoFlowEvent {
    private static Logger logger = LoggerFactory.getLogger();

    @Override
    public AutoFlowEventResponse execute(AutoFlowEventRequest autoFlowEventRequest) throws ScriptBusinessException {

        AutoFlowEventResponse response = new AutoFlowEventResponse();

        Long inquiryId = autoFlowEventRequest.getDataId();                 //流里面的数据
        List<String> idS = new ArrayList<>();
        Set<Long> accIdS=new HashSet<>();
        List<Long> id = new ArrayList<Long>();
        id.add(inquiryId);
        try {
            List<Inquiry__c> inquiryList = XObjectService.instance().getByIds("inquiry__c",id);
            ...
            if(!idS.isEmpty()){
                ...
                if(json.getJSONObject("response").getJSONObject("fields").getString("msg").equals("操作成功")){
                    ...
                    OperateResult uresult = XObjectService.instance().update(updateInq);
