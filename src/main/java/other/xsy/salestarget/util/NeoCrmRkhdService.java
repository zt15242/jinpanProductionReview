...
public class NeoCrmRkhdService {
    ...
    */
    ...
    /***
    * @Description: 根据对象、控制字段和被依赖字段 ID 查询某一对字段依赖关系。
    * @Param: [client, xobjectApiKey, controlItemApiKey, dependentItemApiKey]
    * @return: java.util.Map<java.lang.String,java.util.List<java.lang.String>>
    * @Author: 武于伦
    * @email: 2717718875@qq.com
    * @Date: 2025/9/25
    */
    public static Map<String,List<String>> getControllingValue(RkhdHttpClient client,String xobjectApiKey,String controlItemApiKey,String dependentItemApiKey){
        ...
        try {
            ...
            RkhdHttpData data = new RkhdHttpData();
            data.setCallString("/rest/metadata/v2.0/xobjects/"+xobjectApiKey+"/itemDependencies/itemItems/controlItems/"+controlItemApiKey+"/"+dependentItemApiKey);
            data.setCall_type("GET");
            String responseStr = client.execute(data, ResponseBodyHandlers.ofString());
            if (StringUtils.isNotBlank(responseStr)) {
                logger.info("查询列表结果：" + responseStr);
                ...
                Integer responseCode = responseObject.getIntValue("code");
                if (responseCode.equals(0)) {
                    JSONArray jsonArray = responseObject.getJSONObject("data")
                                                        .getJSONObject("records")
                                                        .getJSONArray("itemDependencies");
                    if (jsonArray.size() > 0) {
                        JSONArray jsonArray1 = jsonArray.getJSONObject(0)
                                .getJSONArray("itemDependencyDetailAOs");
                        if (jsonArray1.size() > 0) {
                            for (int i = 0; i < jsonArray1.size(); i++) {
                                JSONObject jsonObject = jsonArray1.getJSONObject(i);
                                String label = jsonObject.getString("controlItemCode");
                                String value = jsonObject.getString("dependentItemCodeList");
                                if (!stringListHashMap.containsKey(label)) {
                                    stringListHashMap.put(label,Arrays.asList(value.split(",")));
                                }
                            }
                        }
                    }
...
//        OperateResult upsert = upsert(null, receiptAndInspectionNote__c, "deliveryNo__c", true, true);
        Map<String, List<String>> controllingValue = getControllingValue(null, "salesTarget__c", "business__c", "product_Type__c");
